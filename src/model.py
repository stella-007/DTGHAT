...
class MDA(nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        self.mlp = nn.Sequential(nn.Linear(2648 * 2, 1024),
                                 nn.Linear(1024, 512),
                                 nn.Linear(512, 64),
    ...
    def forward(self, data, train_sample, test_sample, device):
        ...
        for i in range(len(train_sample)):
            a = torch.cat((data['inter_drug_protein_feature'][train_sample[i][0]], data['inter_drug_protein_feature'][train_sample[i][1]]), dim=0).unsqueeze(0)
        ...
        for i in range(len(test_sample)):
            a = torch.cat((data['inter_drug_protein_feature'][test_sample[i][0]], data['inter_drug_protein_feature'][test_sample[i][1]]), dim=0).unsqueeze(0)
